def user_recommendation():
    ...
    # Get combined features for users speciifed with user_id
    def get_combined_feature(user):
        feat = df[df.user_id==user].combined_features
        return feat.unique()[0]
    
    # Get the index of a user from the tables given
    def get_index(f,act):
        return f[f.user_id==act].index.values

    # Get the total actions count for a user's posts   
    ...
    #Recommends posts due to user preferences or suggest if new user
    def user_recommendation_base (user):
        ...
        return [int(i) for i in cb]

    #analyse user and recommend posts to user   
    ...
    output = [int(i) for i in user_recommed(user)]
    print('\t',"Top 10 Recommended Users",'\n',output)
    for i in output:
        print('\t', 'id: ',i,', name:', get_name_from_user_id(i))
